...
export default defineConfig({
  ...
  vite: {
    plugins: [
      tailwindcss(),
      {
        name: "mock-node-modules-client",
        enforce: "pre",
        resolveId(id, importer, options) {
          // 仅在客户端构建时拦截
          if (options?.ssr === false) {
            if (id === "node:fs" || id === "node:path" || id === "node:url") {
              return "\0" + id;
            }
          }
        ...
        load(id) {
          ...
          if (id === "\0node:path") {
            return `export function dirname() {} export function join() {}`;
          }
          if (id === "\0node:url") {
            return `export function fileURLToPath() {}`;
          }
        },
      },
    ],
